...
print(stocktray)
ROWS = len(stocktray)
tray = [i for i in range(0,len(stocktray))]
df = pd.DataFrame(index=tray)
profit = 99
#BETA = 98
#ATR = 97
df[profit] = [0 for _ in range(len(df))]
#df[BETA] = [0 for _ in range(len(df))]
#df[ATR] = [0 for _ in range(len(df))]
count = 0
company_df = []
for v, index in enumerate(stocktray[0:ROWS]):  
    print(count)
    try:
        df.loc[v, profit] = TryStock(index)
        company = yf.Ticker(index)
        company_info = company.info
        #company_df = pd.DataFrame.from_dict(company_info, orient='index', columns=['Value'])
        company_df.append(company_info)
        print(f'company {index} has been added')
        #df.loc[index, BETA] = company.info['beta']
        #df.loc[index, ATR] = company.info['ATR']
        #df[index, BETA] = company_df[index, 'beta']
    except Exception as e:
        print('Exception: ', e)
        continue
    count+=1
...
column_to_add = df.iloc[:ROWS, 0]
#print(column_to_add)
company_df['Profit'] = column_to_add


#print(company_df)
#merged_df = pd.concat([df.iloc[:5], company_df.iloc[:, 1:]], axis=0)
#print(merged_df)
#%%
Metrics = ['beta', 'pegRatio', 'trailingPE', 'trailingEps', 'forwardPE', 'averageVolume', 'marketCap', 'revenuePerShare', 'returnOnAssets', 'returnOnEquity', 'debtToEquity', 'cashPerShare', 'grossProfit', 'recommendationMean', 'priceToSalesTrailing12Months', 'fullTimeEmployees', 'overallRisk', 'shortRatio', 'priceToBook']

def merticCorr(metric):
    x = company_df[metric]
    x.fillna(0, inplace=True)
    y = company_df['Profit']
    y.fillna(0, inplace=True)
    correlation_coefficient = np.corrcoef(x, y)[0, 1]
    print(f'Correlation Coefficient with {metric}: {correlation_coefficient:.2f}')
    #
    y_log = np.log(company_df['Profit'])
    plt.xlabel(metric)
    plt.ylabel('PROFIT ($)')
    plt.title(f'{metric} vs PROFIT')
    plt.scatter(x, y_log)
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
AverageProfit = np.mean(df[profit])
print(f'Average Profit: {AverageProfit:.2f}$')


#%%
df = pd.DataFrame(index=[1, 2])
company = yf.Ticker('AAPL')
df.append(pd.DataFrame(company.info))
print(df)
